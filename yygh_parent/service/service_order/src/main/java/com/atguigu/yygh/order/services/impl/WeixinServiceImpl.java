 ...
 * @updateRemark
 * @updateUser
 * @createDate 2021/6/9 10:39
 * @updateDate 2021/6/9 10:39
 **/
@Service
public class WeixinServiceImpl implements WeixinService {
    ...
    @Autowired
    private RefundInfoService refundInfoService = null;
    ...
    public Map createNative(Long orderId) throws Exception {
        ...
        if (resultMap.get("result_code") != null) {
            redisTemplate.opsForValue().set(orderId.toString(), map, 120, TimeUnit.SECONDS);
        }
        return map;
    }
    ...
    public Boolean refund(Long orderId) {
        try {
            ...
            paramMap.put("nonce_str", WXPayUtil.generateNonceStr());
            paramMap.put("transaction_id", paymentInfo.getCallbackContent());//微信订单号
            paramMap.put("out_trade_no", paymentInfo.getOutTradeNo());//商户订单编号
            paramMap.put("out_refund_no", "tk" + paymentInfo.getOutTradeNo());//商户退款单号
            paramMap.put("total_fee", "1");
            paramMap.put("refund_fee", "1");
            String signedXml = WXPayUtil.generateSignedXml(paramMap, ConstantPropertiesUtils.PARTNERKEY);
